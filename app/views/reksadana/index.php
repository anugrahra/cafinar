<div class="row">
  <div class="col">
    <h3>REKSADANA <span class="text-secondary">(Rp <?= number_format($data['total']['total']); ?>)</span></h3>
  </div>
...
<div class="row">
  ...
  <div class="col d-flex justify-content-end">
    <form class="form-inline" method="POST" action="<?= BASEURL; ?>/reksadana/index">
  ...
  <div class="col">
    <table class="table table-bordered table-hover">
      ...
      <tbody>
        <?php
        $no = 1;
        foreach ($data['reksadana'] as $reksadana) :
        ?>
          <tr>
            <td><?= $no++; ?></td>
            <td><?= date('d F Y', strtotime($reksadana['tanggal'])); ?></td>
            <td><?= number_format($reksadana['invest']); ?></td>
            <td><?= number_format($reksadana['jual']); ?></td>
            <td><?= $reksadana['keterangan']; ?></td>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
